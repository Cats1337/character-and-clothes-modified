...
local exitM = Material("materials/clothesmod/exit.png")
local pay = Material("materials/clothesmod/pay.png")
local edit = Material("materials/clothesmod/edit.png")
local photo = Material("materials/clothesmod/photo.png")
local ent
...
local function OpenPreview(Panel)
    ...
    local data = CLOTHESMOD.Male.EditableTop[texture]
...
local function CreateImageGUI(parent)
    ...
    dhtml:SetPos(0, 0)
    dhtml:SetSize(50, 50)
    dhtml.Link = "https://www.pngall.com/wp-content/uploads/2016/03/Cat-PNG-6.png" -- image was broken so I found this random cat picture to replace it :) -- Cats
    dhtml.SizeX = 50
    dhtml.SizeY = 50
    dhtml:SetHTML([[
...
local function OpenImage(Panel, Panel2)
    ...
    DPanel.Paint = function(pnl, w, h) end
    ...
    button.Paint = function(pnl, w, h)
        ...
        draw.SimpleText("> " .. CLOTHESMOD.Config.Sentences[1][CLOTHESMOD.Config.Lang], "Bariol20", w / 2 - 5, h / 2, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, 1)
    end

    local DLabel = vgui.Create("DLabel", DPanel)
    surface.SetFont("Bariol25")
    local x, y = surface.GetTextSize(CLOTHESMOD.Config.Sentences[2][CLOTHESMOD.Config.Lang])
    DLabel:SetSize(x, y)
    DLabel:SetPos((sizex - 4) / 2 - x / 2, y + 10)
    DLabel:SetFont("Bariol25")
    DLabel:SetText(CLOTHESMOD.Config.Sentences[1][CLOTHESMOD.Config.Lang])
    ...
    for k, v in pairs(CustomTop.Images) do
        ...
        panelM1.Paint = function(pnl, w, h)
            local we = 2
            draw.RoundedBox(0, 0, 0, w, h, Color(0, 0, 0, 150))
            draw.SimpleText(CLOTHESMOD.Config.Sentences[3][CLOTHESMOD.Config.Lang] .. " " .. k, "Bariol25", w / 2, 15, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, 1)
        ...
        button.Paint = function(pnl, w, h)
            ...
            draw.RoundedBox(0, 0, 0, we, h, Color(255, 255, 255, 255))
            draw.RoundedBox(0, w - we, 0, we, h, Color(255, 255, 255, 255))
            draw.SimpleText("> " .. CLOTHESMOD.Config.Sentences[4][CLOTHESMOD.Config.Lang], "Bariol20", w / 2 - 5, h / 2, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, 1)
        end
...
local function OpenText(Panel, Panel2)
    ...
    local DLabel = vgui.Create("DLabel", DPanel)
    surface.SetFont("Bariol25")
    local x, y = surface.GetTextSize(CLOTHESMOD.Config.Sentences[5][CLOTHESMOD.Config.Lang])
    DLabel:SetSize(x, y)
    DLabel:SetPos((sizex - 4) / 2 - x / 2, y + 10)
    DLabel:SetFont("Bariol25")
    DLabel:SetText(CLOTHESMOD.Config.Sentences[5][CLOTHESMOD.Config.Lang])
    DLabel:SetTextColor(Color(255, 255, 255))
    sizex = sizex + 160
    local nb = 1
    nbtxt = 0
    ...
    for k, v in pairs(CustomTop.Texts) do
        ...
        panelM1.Paint = function(pnl, w, h)
            local we = 2
            draw.RoundedBox(0, 0, 0, w, h, Color(0, 0, 0, 150))
            draw.SimpleText(CLOTHESMOD.Config.Sentences[6][CLOTHESMOD.Config.Lang] .. " " .. k, "Bariol25", w / 2, 15, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, 1)
        end
...
local function OpenTee(Panel, Panel2)
    local DPanel = vgui.Create("DScrollPanel", Panel)
    -- title for shirt selection -- Cats
    ...
    local shirtList = {} -- Cats
    local infos = LocalPlayer():CM_GetInfos()

    if infos.sex == 1 then
        shirtList = CLOTHESMOD.Male.EditableTop
    else
        shirtList = CLOTHESMOD.Female.EditableTop
    end
    ...
    -- Cats
    for tee, tables in pairs(shirtList) do
        local nbl = math.fmod(nb, mod)
        ...
        dmodelPanel.Actualize = function()
            ...
            if infos.sex == 1 then
                datas = CLOTHESMOD.Male
            else
                datas = CLOTHESMOD.Female
            end

            local texture = tee
            local bodygroupname = datas.EditableTop[texture].bodygroup

            local bodygroups = {datas.ListDefaultPM[LocalPlayer():CM_GetInfos().model].bodygroupstop[bodygroupname].group}
        ...
        function dmodelPanel.Entity:GetPlayerColor()
            return Vector(1, 1, 1)
        end
...
local function OpenPay(Panel, Panel2)
    ...
    DPanel.Paint = function(pnl, w, h)
        draw.SimpleText(CLOTHESMOD.Config.Sentences[8][CLOTHESMOD.Config.Lang], "Bariol25", w / 2, 30, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER)
        draw.SimpleText(CLOTHESMOD.Config.Sentences[9][CLOTHESMOD.Config.Lang], "Bariol20", w / 2, 60, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER)

        if err ~= "" then
            draw.SimpleText(CLOTHESMOD.Config.Sentences[10][CLOTHESMOD.Config.Lang] .. " : " .. err, "Bariol20", w / 2, 110 + 10 + 30 + 40, Color(255, 0, 0, 255), TEXT_ALIGN_CENTER)
        end
    end
    ...
    function button:DoClick()
        if nbtxt <= 0 and nbimg <= 0 then
            err = CLOTHESMOD.Config.Sentences[11][CLOTHESMOD.Config.Lang]
        ...
        for k, v in pairs(CLOTHESMOD.ShopTextures) do
            if v.name and v.name == CustomTop.name then
                err = CLOTHESMOD.Config.Sentences[12][CLOTHESMOD.Config.Lang]
        ...
        MFrame:Remove()
        net.Start("ClothesMod:CustomTeeCreationShop")
    ...
    button.Paint = function(pnl, w, h)
        ...
        draw.RoundedBox(0, 0, 0, we, h, Color(255, 255, 255, 255))
        draw.RoundedBox(0, w - we, 0, we, h, Color(255, 255, 255, 255))
        draw.SimpleText("> " .. CLOTHESMOD.Config.Sentences[13][CLOTHESMOD.Config.Lang], "Bariol20", w / 2 - 5, h / 2, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, 1)
    end
end
...
local function OpenEdit(Panel)
    ...
    -- local DPanelMenu = vgui.Create( "DScrollPanel",DPanelMen )
...
function CLOTHESMOD_OpenAdminGui()
